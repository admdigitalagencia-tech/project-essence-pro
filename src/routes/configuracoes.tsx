    ...
    <PageContainer>
      ...
      <Card className="p-5 mt-4 border-border/60 shadow-none">
        ...
        <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
          <Range label="0 — 4.9" tone="bg-muted" desc="Baixo valor operacional" />
          <Range label="5 — 6.9" tone="bg-accent" desc="Tarefa comum" />
  ...
  return (
    <Card className="p-5 border-border/60 shadow-none">
      ...
      <div className="flex flex-wrap gap-1.5">
        {items.map((i) => (
          <span key={i} className="text-xs px-2.5 py-1 rounded-md bg-muted text-muted-foreground">{i}</span>
        ))}
      </div>
    </Card>
  );
